...
std::vector<int> solidTiles = { 0, 1, 2, 5, 6, 16, 17, 18, 19, 32, 33, 34, 35 };
std::vector<int> dangerTiles = { 100, 101 };
std::vector<int> climbTile = { 6 };

//int Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 4096);
...
public:
    ...
    bool Update(float elapsed, FlareMap* map) {
        ...
        if (wallJumpFrames > 0.8f) {
            collidedLeft = collidedRight = wallJump = false;
            wallJumpFrames = 0.0f;
        }
        ...
        if (event.type == SDL_QUIT || event.type == SDL_WINDOWEVENT_CLOSE) {
            ...
            if (event.key.keysym.scancode == SDL_SCANCODE_A) {
                if (player.wallJump && player.collidedLeft) {
                    ...
                    player.velX = TILE_SIZE * 8;
                    player.velY = TILE_SIZE * 4;
                }
            }
    ...
    void SetEntities() {
        switch (mode) {
            case (STATE_GAME_LEVEL1):
                ...
                victory = Entity(
                                 level1.entities[2].x + (TILE_SIZE), level1.entities[2].y + (TILE_SIZE),
                                 TILE_SIZE, TILE_SIZE, false
                                 );
            ...
            case (STATE_GAME_LEVEL2):
                ...
                victory = Entity(
                                 level2.entities[1].x + (TILE_SIZE), level2.entities[1].y + (TILE_SIZE),
                                 TILE_SIZE, TILE_SIZE, false
                                 );
                victory.sheet = Texture;
                victory.spriteIndex = 0;
            ...
            case (STATE_GAME_LEVEL3):
                ...
                victory = Entity(
                                 level3.entities[11].x + (TILE_SIZE), level3.entities[11].y + (TILE_SIZE),
                                 TILE_SIZE, TILE_SIZE, false
                                 );
                victory.sheet = Texture;
                victory.spriteIndex = 0;
